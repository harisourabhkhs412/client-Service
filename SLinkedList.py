...
class SLinkedList:
    ...
    def insert(self,pos,item):  
        assert type(pos)==int,"pos is not an int type" 
        assert pos>=0, "pos cannot be negative"
        
        current = self.head
        new_node = SLinkedListNode(item,None)     
        i = 1
        
        if pos==0: #adding node to beg of list
            self.add(item)
        elif pos==self.size: #adding node to end of the list, when current is equal to last item in list we set next to new node and increment the size by 1
            while (current.getNext()!=None): #node that is pointing to none
                current= current.getNext() # traversing the list, one before the last node
            current.setNext(new_node) #stop before reaching last node
            self.size = self.size +1          
